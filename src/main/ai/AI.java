...
public class AI {
  ...
  private int minimax(Player curr, Player opp, int depth, int alpha, int beta, boolean isMaxing) {
    ...
    if (depth == 0) {
      return evaluatePos(curr, opp, isMaxing);
    }
    if (isMaxing) {
  ...
  private int evaluatePos(Player aiPlayer, Player oppPlayer, boolean isAITurn) {
    ...
    // Evaluate a representative move (or average a few)
    int totalScore = 0;
    int samplesToEval = Math.min(5, aiMoves.size()); // Sample up to 5 moves

    for (int i = 0; i < samplesToEval; i++) {
      Move m = aiMoves.get(i);
      int score = useH2 ? h.evalH2(aiPlayer, oppPlayer, m) : h.evalH1(aiPlayer, oppPlayer, m);
      totalScore += score;
    }
